        ...
        <div class="max-h-80 lg:max-h-96 overflow-y-auto">
            @if(count($notifications) > 0)
                @foreach($notifications as $notification)
                    <div
                        class="px-4 py-3 border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer transition-colors {{ $notification['read_at'] ? 'opacity-75' : 'bg-blue-50 dark:bg-blue-900/20' }}"
                        wire:click="markAsRead('{{ $notification['id'] }}')"
                    >
                        <div class="flex items-start space-x-3">
                            <!-- Notification Icon -->
                            <div class="flex-shrink-0 mt-1">
                                @if(str_contains($notification['type'], 'FollowUpReminder'))
                                    <div class="w-2 h-2 bg-orange-500 rounded-full"></div>
                                @elseif(str_contains($notification['type'], 'ActivityReminder'))
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                @if(isset($notification['data']['lead_title']))
                                    <p class="text-xs text-gray-600 dark:text-gray-400 mt-1">
                                        Lead: {{ $notification['data']['lead_title'] }}
                                    </p>
                                @endif

                                @if(isset($notification['data']['activity_subject']))
                                    <p class="text-xs text-gray-600 dark:text-gray-400 mt-1">
                                        Activity: {{ $notification['data']['activity_subject'] }}
                                    </p>
                                @endif
